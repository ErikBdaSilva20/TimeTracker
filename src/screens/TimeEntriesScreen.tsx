    ...
    <>
      ...
      <PageBody>
        <div className="flex flex-wrap items-center gap-3">
          <select
            value={filter.client ?? ""}
            onChange={(e) => setFilter((f) => ({ ...f, client: e.target.value || undefined }))}
          ...
          <div className="ml-auto text-sm text-[var(--text-muted)]">
            ...
            <span className="text-[var(--text-primary)] font-medium">{formatHours(total)}</span>
          </div>
        </div>
        ...
        {entriesQ.isLoading ? (
          ...
          <EmptyState
            title="Sem registros"
            description="Registre horas via Timer ou manualmente."
            action={
              <button
                onClick={() => setShowForm(true)}
              ...
              <tbody>
                {entries.map((e) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      {e.billable ? <Badge tone="primary">sim</Badge> : <Badge>não</Badge>}
                    </td>
                    <td className="px-5 py-3 text-right">
                      <button
                        onClick={async () => {
                          if (!(await confirm("Excluir?"))) return;
                          await runMutation(() => timeEntriesRepo.remove(e.id), {
